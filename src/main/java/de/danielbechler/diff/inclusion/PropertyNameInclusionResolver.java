...
class PropertyNameInclusionResolver implements InclusionResolver
{
	...
	public Inclusion getInclusion(final DiffNode node)
	{
		...
		{
			final Inclusion inclusion = propertyNameInclusions.get(propertyName);
			if (EnumSet.of(INCLUDED, EXCLUDED).contains(inclusion))
			{
				return inclusion;
			}
		}
		if (node.getParentNode() != null && getInclusion(node.getParentNode()) == INCLUDED)
		{
			return INCLUDED;
		}
		return DEFAULT;
